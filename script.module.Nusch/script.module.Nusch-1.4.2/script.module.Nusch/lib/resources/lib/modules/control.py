...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Nusch.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Nusch.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Nusch.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Nusch.artwork)')
...
def moderator():
    netloc = [urlparse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.chappaai', 'plugin.video.live.streamspro', 'plugin.video.phstreams', 'plugin.video.cpstreams', 'plugin.video.tinklepad', 'script.tvguide.nusch', 'script.tvguide.assassins']

    if not infoLabel('Container.PluginName') in netloc: sys.exit()

def metaFile():
    if condVisibility('System.HasAddon(script.Nusch.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Nusch.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
