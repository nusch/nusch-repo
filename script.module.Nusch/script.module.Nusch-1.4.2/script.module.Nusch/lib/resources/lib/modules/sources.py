...
class sources:
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.Nusch.container.items'

        self.metaProperty = 'plugin.video.Nusch.container.meta'
        scraperSetting = control.setting('module.provider')

        try:
            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.lambdascrapers') and scraperSetting == 'Civitas_Scrapers':
                from civitasscrapers import sources
                self.sourceDict = sources()
                self.module_name = control.addon('script.module.civitasscrapers').getSetting('module.provider')  

            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.lambdascrapers') and scraperSetting == 'Lambda_Scrapers':
                from lambdascrapers import sources
                self.sourceDict = sources()
                self.module_name = control.addon('script.module.lambdascrapers').getSetting('module.provider')

            if xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.nuschscrapers') and scraperSetting == 'Nusch_Scrapers':
                from nuschscrapers import sources
                self.sourceDict = sources()
                self.module_name = control.addon('script.module.nuschscrapers').getSetting('module.provider')

            if scraperSetting == 'Default':
